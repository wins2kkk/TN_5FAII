using System;
using System.Collections.Generic;
using System.Linq;
using UnityEngine;
using UnityEngine.Events;
using UnityEngine.UI;


namespace Gley.DailyRewards.Internal
{
    public class CalendarManager : MonoBehaviour
    {
        private UnityAction<int, int, Sprite> ClickListener;
        private List<CalendarDayProperties> allDays;
        private GameObject canvas;
        private GameObject popup;
        private DateTime savedTime;
        private TimeSpan timeToPass;
        private int savedDay;
        private bool resetAtEnd;
        private bool initialized;
        private ValueFormatterFunction valueFormatterFunction = null;
        const string dailyRewardSavedTime = "DailyRewardSavedTime";
        const string dailyRewardCurrentDay = "DailyRewardCurrentDay";

        private static GameObject go;
        private static CalendarManager instance;

        public static CalendarManager Instance
        {
            get
            {
                if (instance == null)
                {
                    go = new GameObject();
                    go.name = "CalendarManager";
                    DontDestroyOnLoad(go);
                    instance = go.AddComponent<CalendarManager>();
                    instance.Initialize();
                }
                return instance;
            }
        }




        /// <summary>
        /// Loads the saved values
        /// </summary>
        void Initialize()
        {
            if (initialized == false)
            {
                initialized = true;
                DailyRewardsData dailyRewardsSettings = Resources.Load<DailyRewardsData>(Constants.DATA_NAME_RUNTIME);
                if (dailyRewardsSettings == null)
                {
                    Debug.LogWarning("Daily Rewards is not setup. Go to Tools->Gley->Daily Rewards to setup");
                }
                else
                {
                    canvas = dailyRewardsSettings.calendarCanvas;
                    popup = dailyRewardsSettings.calendarPrefab;
                    allDays = dailyRewardsSettings.allDays;
                    timeToPass = new TimeSpan(dailyRewardsSettings.hours, dailyRewardsSettings.minutes, dailyRewardsSettings.seconds);
                    resetAtEnd = dailyRewardsSettings.resetAtEnd;

                    if (dailyRewardsSettings.availableAtStart == true)
                    {
                        if (!TimeMethods.SaveExists(dailyRewardSavedTime))
                        {
                            TimeMethods.MakeButtonAvailable(dailyRewardSavedTime, timeToPass);
                        }
                    }

                    savedTime = TimeMethods.LoadTime(dailyRewardSavedTime);
                    savedDay = TimeMethods.LoadDay(dailyRewardCurrentDay);
                }
            }
        }

        public void NextDay()
        {
            GoToNextDay();
        }

        public void SetValueFormatter(ValueFormatterFunction aFunction)
        {
            valueFormatterFunction = aFunction;
        }

        /// <summary>
        /// Resets calendar from day one
        /// </summary>
        public void ResetCalendar()
        {
            TimeMethods.ResetTime(dailyRewardSavedTime);
            TimeMethods.ResetDay(dailyRewardCurrentDay);
            initialized = false;
        }


        /// <summary>
        /// Display the Calendar Popup
        /// </summary>
        public void ShowCalendar()
        {
            if (canvas == null)
            {
                Debug.LogWarning("canvas cannot be null. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }

            if (popup == null)
            {
                Debug.LogWarning("popup cannot be null. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }

            if (allDays.Count == 0)
            {
                Debug.LogWarning("You need at least one day. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }
            LoadPopup(canvas, popup, allDays);
        }


        /// <summary>
        /// Get the remaining formatted time until the next reward is ready
        /// </summary>
        /// <returns>Formated time</returns>
        public string GetRemainingTime()
        {
            TimeSpan difference = GetRemainingTimeSpan();
            return string.Format("{0:D2}:{1:D2}:{2:D2}", difference.Hours, difference.Minutes, difference.Seconds);
        }


        /// <summary>
        /// Get the remaining Time Span until the next reward is ready
        /// </summary>
        /// <returns></returns>
        public TimeSpan GetRemainingTimeSpan()
        {
            TimeSpan difference = TimeMethods.SubtractTime(savedTime);
            difference = timeToPass.Subtract(difference);
            if (TimeExpired())
            {
                difference = new TimeSpan(0, 0, 0);
            }
            return difference;
        }


        /// <summary>
        /// Check if the current reward is available to claim
        /// </summary>
        /// <returns>true if available</returns>
        public bool TimeExpired()
        {
            TimeSpan difference = TimeMethods.SubtractTime(savedTime);
            if ((int)difference.TotalSeconds / (int)timeToPass.TotalSeconds > 0)
            {
                return true;
            }
            return false;
        }


        /// <summary>
        /// Returns the current active day number
        /// </summary>
        /// <returns></returns>
        public int GetCurrentDay()
        {
            return savedDay;
        }


        /// <summary>
        /// Register a listener for day click
        /// </summary>
        /// <param name="ClickListener">needs the following params (dayNumber, rewardValue, rewardText, rewardSprite)</param>
        public void AddClickListener(UnityAction<int, int, Sprite> ClickListener)
        {
            this.ClickListener = ClickListener;
        }


        /// <summary>
        /// Called when a day button is clicked
        /// </summary>
        /// <param name="dayNumber">current day clicked</param>
        /// <param name="rewardValue">current reward</param>
        /// <param name="rewardSprite">button sprite</param>
        public void ButtonClick(int dayNumber, int rewardValue, Sprite rewardSprite)
        {
            GoToNextDay();
            if (ClickListener != null)
            {
                ClickListener(dayNumber, rewardValue, rewardSprite);
            }
        }

        void GoToNextDay()
        {
            savedDay++;
            if (savedDay == allDays.Count)
            {
                if (resetAtEnd)
                {
                    savedDay = 0;
                }
                else
                {
                    savedDay = allDays.Count - 1;
                }
            }
            TimeMethods.SaveDay(dailyRewardCurrentDay, savedDay);
            TimeMethods.SaveTime(dailyRewardSavedTime);
            savedTime = TimeMethods.LoadTime(dailyRewardSavedTime);
            FindObjectsByType<CalendarPopup>(FindObjectsSortMode.None)[0].Refresh(savedDay, false);
        }


        /// <summary>
        /// Loads the Calendar Popup on stage
        /// </summary>
        /// <param name="canvas">canvas prefab</param>
        /// <param name="popup">popup prefab</param>
        /// <param name="allDays">list of all days to load</param>
        private void LoadPopup(GameObject canvas, GameObject popup, List<CalendarDayProperties> allDays)
        {
            Canvas[] allCanvases = FindObjectsByType<Canvas>(FindObjectsSortMode.None);
            int max = 1;
            if (allCanvases.Length > 0)
            {
                max = allCanvases.Max(cond => cond.sortingOrder);
            }

            Transform calendarPopupCanvas = Instantiate(canvas).transform;
            max++;
            if (max < 0)
            {
                max = short.MaxValue;
            }
            calendarPopupCanvas.GetComponent<Canvas>().sortingOrder = max;
            if (Screen.width > Screen.height)
            {
                calendarPopupCanvas.GetComponent<CanvasScaler>().referenceResolution = new Vector2(1920, 1080);
            }
            else
            {
                calendarPopupCanvas.GetComponent<CanvasScaler>().referenceResolution = new Vector2(1080, 1920);
            }

            Transform calendarPopup = Instantiate(popup).transform;
            calendarPopup.SetParent(calendarPopupCanvas, false);
            calendarPopup.GetComponent<CalendarPopup>().Initialize(allDays, savedDay, TimeExpired(), valueFormatterFunction);
        }
    }
}
//using System;
//using System.Collections.Generic;
//using System.Linq;
//using UnityEngine;
//using UnityEngine.Events;
//using UnityEngine.UI;

//namespace Gley.DailyRewards.Internal
//{
//    public class CalendarManager : MonoBehaviour
//    {
//        private UnityAction<int, int, Sprite> ClickListener;
//        private List<CalendarDayProperties> allDays;
//        private GameObject canvas;
//        private GameObject popup;
//        private DateTime savedTime;
//        private TimeSpan timeToPass;
//        private int savedDay;
//        private bool resetAtEnd;
//        private bool initialized;
//        private bool serverTimeSynced = false;
//        private ValueFormatterFunction valueFormatterFunction = null;

//        const string dailyRewardSavedTime = "DailyRewardSavedTime";
//        const string dailyRewardCurrentDay = "DailyRewardCurrentDay";

//        private static GameObject go;
//        private static CalendarManager instance;

//        public static CalendarManager Instance
//        {
//            get
//            {
//                if (instance == null)
//                {
//                    go = new GameObject();
//                    go.name = "CalendarManager";
//                    DontDestroyOnLoad(go);
//                    instance = go.AddComponent<CalendarManager>();
//                    instance.InitializeWithServerSync();
//                }
//                return instance;
//            }
//        }

//        /// <summary>
//        /// Initialize với server time sync
//        /// </summary>
//        void InitializeWithServerSync()
//        {
//            // Sync server time trước khi initialize
//            TimeMethods.Initialize(
//                onComplete: () => {
//                    serverTimeSynced = true;
//                    Initialize();
//                    Debug.Log("Calendar Manager initialized with server time sync");
//                },
//                onError: (error) => {
//                    Debug.LogWarning($"Server time sync failed, using local time: {error}");
//                    serverTimeSynced = false;
//                    Initialize(); // Vẫn initialize với local time
//                }
//            );
//        }

//        /// <summary>
//        /// Loads the saved values
//        /// </summary>
//        void Initialize()
//        {
//            if (initialized == false)
//            {
//                initialized = true;
//                DailyRewardsData dailyRewardsSettings = Resources.Load<DailyRewardsData>(Constants.DATA_NAME_RUNTIME);
//                if (dailyRewardsSettings == null)
//                {
//                    Debug.LogWarning("Daily Rewards is not setup. Go to Tools->Gley->Daily Rewards to setup");
//                }
//                else
//                {
//                    canvas = dailyRewardsSettings.calendarCanvas;
//                    popup = dailyRewardsSettings.calendarPrefab;
//                    allDays = dailyRewardsSettings.allDays;
//                    timeToPass = new TimeSpan(dailyRewardsSettings.hours, dailyRewardsSettings.minutes, dailyRewardsSettings.seconds);
//                    resetAtEnd = dailyRewardsSettings.resetAtEnd;

//                    if (dailyRewardsSettings.availableAtStart == true)
//                    {
//                        if (!TimeMethods.SaveExists(dailyRewardSavedTime))
//                        {
//                            TimeMethods.MakeButtonAvailable(dailyRewardSavedTime, timeToPass);
//                        }
//                    }

//                    savedTime = TimeMethods.LoadTime(dailyRewardSavedTime);
//                    savedDay = TimeMethods.LoadDay(dailyRewardCurrentDay);
//                }
//            }
//        }

//        public void NextDay()
//        {
//            GoToNextDay();
//        }

//        public void SetValueFormatter(ValueFormatterFunction aFunction)
//        {
//            valueFormatterFunction = aFunction;
//        }

//        /// <summary>
//        /// Resets calendar from day one
//        /// </summary>
//        public void ResetCalendar()
//        {
//            TimeMethods.ResetTime(dailyRewardSavedTime);
//            TimeMethods.ResetDay(dailyRewardCurrentDay);
//            initialized = false;

//            // Re-sync server time sau khi reset
//            InitializeWithServerSync();
//        }

//        /// <summary>
//        /// Display the Calendar Popup
//        /// </summary>
//        public void ShowCalendar()
//        {
//            // Auto resync server time nếu cần
//            TimeMethods.AutoResyncIfNeeded();

//            if (canvas == null)
//            {
//                Debug.LogWarning("canvas cannot be null. Go to Tools->Gley->Daily Rewards to setup");
//                return;
//            }

//            if (popup == null)
//            {
//                Debug.LogWarning("popup cannot be null. Go to Tools->Gley->Daily Rewards to setup");
//                return;
//            }

//            if (allDays.Count == 0)
//            {
//                Debug.LogWarning("You need at least one day. Go to Tools->Gley->Daily Rewards to setup");
//                return;
//            }
//            LoadPopup(canvas, popup, allDays);
//        }

//        /// <summary>
//        /// Get the remaining formatted time until the next reward is ready
//        /// </summary>
//        /// <returns>Formated time</returns>
//        public string GetRemainingTime()
//        {
//            TimeSpan difference = GetRemainingTimeSpan();
//            return string.Format("{0:D2}:{1:D2}:{2:D2}", difference.Hours, difference.Minutes, difference.Seconds);
//        }

//        /// <summary>
//        /// Get the remaining Time Span until the next reward is ready
//        /// </summary>
//        /// <returns></returns>
//        public TimeSpan GetRemainingTimeSpan()
//        {
//            TimeSpan difference = TimeMethods.SubtractTime(savedTime);
//            difference = timeToPass.Subtract(difference);
//            if (TimeExpired())
//            {
//                difference = new TimeSpan(0, 0, 0);
//            }
//            return difference;
//        }

//        /// <summary>
//        /// Check if the current reward is available to claim
//        /// </summary>
//        /// <returns>true if available</returns>
//        public bool TimeExpired()
//        {
//            TimeSpan difference = TimeMethods.SubtractTime(savedTime);
//            if ((int)difference.TotalSeconds / (int)timeToPass.TotalSeconds > 0)
//            {
//                return true;
//            }
//            return false;
//        }

//        /// <summary>
//        /// Returns the current active day number
//        /// </summary>
//        /// <returns></returns>
//        public int GetCurrentDay()
//        {
//            return savedDay;
//        }

//        /// <summary>
//        /// Register a listener for day click
//        /// </summary>
//        /// <param name="ClickListener">needs the following params (dayNumber, rewardValue, rewardText, rewardSprite)</param>
//        public void AddClickListener(UnityAction<int, int, Sprite> ClickListener)
//        {
//            this.ClickListener = ClickListener;
//        }

//        /// <summary>
//        /// Called when a day button is clicked
//        /// </summary>
//        /// <param name="dayNumber">current day clicked</param>
//        /// <param name="rewardValue">current reward</param>
//        /// <param name="rewardSprite">button sprite</param>
//        public void ButtonClick(int dayNumber, int rewardValue, Sprite rewardSprite)
//        {
//            GoToNextDay();
//            if (ClickListener != null)
//            {
//                ClickListener(dayNumber, rewardValue, rewardSprite);
//            }
//        }

//        void GoToNextDay()
//        {
//            savedDay++;
//            if (savedDay == allDays.Count)
//            {
//                if (resetAtEnd)
//                {
//                    savedDay = 0;
//                }
//                else
//                {
//                    savedDay = allDays.Count - 1;
//                }
//            }
//            TimeMethods.SaveDay(dailyRewardCurrentDay, savedDay);
//            TimeMethods.SaveTime(dailyRewardSavedTime);
//            savedTime = TimeMethods.LoadTime(dailyRewardSavedTime);

//            var popups = FindObjectsByType<CalendarPopup>(FindObjectsSortMode.None);
//            if (popups.Length > 0)
//            {
//                popups[0].Refresh(savedDay, false);
//            }
//        }

//        /// <summary>
//        /// Loads the Calendar Popup on stage
//        /// </summary>
//        /// <param name="canvas">canvas prefab</param>
//        /// <param name="popup">popup prefab</param>
//        /// <param name="allDays">list of all days to load</param>
//        private void LoadPopup(GameObject canvas, GameObject popup, List<CalendarDayProperties> allDays)
//        {
//            Canvas[] allCanvases = FindObjectsByType<Canvas>(FindObjectsSortMode.None);
//            int max = 1;
//            if (allCanvases.Length > 0)
//            {
//                max = allCanvases.Max(cond => cond.sortingOrder);
//            }

//            Transform calendarPopupCanvas = Instantiate(canvas).transform;
//            max++;
//            if (max < 0)
//            {
//                max = short.MaxValue;
//            }
//            calendarPopupCanvas.GetComponent<Canvas>().sortingOrder = max;
//            if (Screen.width > Screen.height)
//            {
//                calendarPopupCanvas.GetComponent<CanvasScaler>().referenceResolution = new Vector2(1920, 1080);
//            }
//            else
//            {
//                calendarPopupCanvas.GetComponent<CanvasScaler>().referenceResolution = new Vector2(1080, 1920);
//            }

//            Transform calendarPopup = Instantiate(popup).transform;
//            calendarPopup.SetParent(calendarPopupCanvas, false);
//            calendarPopup.GetComponent<CalendarPopup>().Initialize(allDays, savedDay, TimeExpired(), valueFormatterFunction);
//        }

//        /// <summary>
//        /// Force resync server time
//        /// </summary>
//        public void ResyncServerTime()
//        {
//            TimeMethods.ForceResync(() => {
//                Debug.Log("Server time resynced successfully");
//                // Refresh saved time with new server time
//                savedTime = TimeMethods.LoadTime(dailyRewardSavedTime);
//            });
//        }

//        /// <summary>
//        /// Check if server time is synced
//        /// </summary>
//        /// <returns></returns>
//        public bool IsServerTimeSynced()
//        {
//            return TimeMethods.IsServerTimeSynced();
//        }

//        /// <summary>
//        /// Get server time offset for debugging
//        /// </summary>
//        /// <returns></returns>
//        public TimeSpan GetServerOffset()
//        {
//            return TimeMethods.GetServerOffset();
//        }

//        // Auto resync khi app resume
//        void OnApplicationPause(bool pauseStatus)
//        {
//            if (!pauseStatus)
//            {
//                TimeMethods.AutoResyncIfNeeded();
//            }
//        }

//        void OnApplicationFocus(bool hasFocus)
//        {
//            if (hasFocus)
//            {
//                TimeMethods.AutoResyncIfNeeded();
//            }
//        }
//    }
//}